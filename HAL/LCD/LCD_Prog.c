...
#if(SPI_MODE_SELECT==ENABLE_SPI_MODE)
...
#elif(SPI_MODE_SELECT==DISABLE_SPI_MODE)
...
void H_Lcd_Void_LCDWriteCharacter(u8 Copy_U8_Character)
{
	...
	M_DIO_Void_SetPinDirection(TRIGGER_PIN, OUTPUT);      // TRIGGER_PIN

	M_DIO_Void_SetPinValue(TRIGGER_PIN, LOW);
	SPI_Write(Copy_U8_Character);
